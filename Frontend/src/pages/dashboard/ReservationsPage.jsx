...
const ReservationsPage = ({ theme }) => {
  ...
  // ============================================
  // HANDLERS PARA CREAR NUEVA RESERVA
  // ============================================
  const handleCreateReservation = () => {
    setSelectedReservation(null); // null = modo crear
    setReservationModalOpen(true);
  };

  const handleSubmitNewReservation = (formData) => {
    console.log("Nueva reserva creada:", formData);
  ...
  // ============================================
  // HANDLERS PARA EDITAR DESDE EL DRAWER
  // ============================================
  const handleEditReservation = (reservation) => {
    setDetailModalOpen(false); // Cerrar drawer
    setSelectedReservation(reservation);
    setReservationModalOpen(true); // Abrir modal de edición
  };

  const handleUpdateReservation = (formData) => {
    console.log("Reserva actualizada:", formData);
    // Aquí harías el PUT a tu API
    // api.put(`/reservations/${selectedReservation.id}`, formData)
  };

  // ============================================
  // HANDLERS PARA CANCELAR/ELIMINAR
  // ============================================
  ...
  const handlePrintInvoice = (reservation) => {
    console.log("Imprimir factura:", reservation);
    // window.print() o generar PDF
  };
  ...
  return (
    <section className={`max-w-7xl mx-auto ${theme.pageText}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-5 mb-6 md:mb-8">
        {kpiCards.map((card) => (
          <Card key={card.title} type="kpi" data={card} isDark={isDark} />
        ))}
      </div>
      ...
      {/* Modal de Detalles (Drawer lateral) */}
      <BookingDetailModal
      ...
      <ReservationModal
        ...
        isDark={isDark}
        reservation={selectedReservation} // null = crear, objeto = editar
        onSubmit={selectedReservation ? handleUpdateReservation : handleSubmitNewReservation}
      />
    </section>
  );
};
